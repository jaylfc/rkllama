...
class RerankEndpointHandler(EndpointHandler):
    ...
    def handle_request(cls, model_name, query, documents, top_n=None, options=None):
        ...
        # Timeout used for each worker reply
        timeout = int(rkllama.config.get("model", "max_seconds_waiting_worker_response"))
        ...
        for idx, doc in enumerate(documents):
            ...
            if parent_pipe.poll(timeout):  # Timeout in seconds
                logits_result = parent_pipe.recv()
            else:
                logger.error(f"Timeout waiting for rerank result for document {idx}")
                # Abort the current inference and return a low score on timeout
                variables.worker_manager_rkllm.workers[model_name].abort_flag.value = True
                parent_pipe.close()
                results.append({"index": idx, "relevance_score": 0.0})
                continue

            # Close the per-request pipe
            parent_pipe.close()
